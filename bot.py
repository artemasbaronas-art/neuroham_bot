...
def activate_user(message):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            bot.send_message(target_id, "💎 **VIP АКТИВОВАНО!**")
            bot.reply_to(message, f"✅ Юзер {target_id} оплатив.")
        except:
...
def main_logic(message):
    ...
    if text in ["📱 ТикТок (VIP)", "🤖 Помощник (VIP)", "🧠 ChatGPT (VIP)"]:
        if uid == ADMIN_ID or uid in paid_users:
            ...
            pay_url = "https://privat24.ua"
            markup.add(types.InlineKeyboardButton(text="💳 ОПЛАТИТИ 50 ГРН", url=pay_url))
            markup.add(types.InlineKeyboardButton(text="📩 ВІДПРАВИТИ ЧЕК", url=f"https://t.me{ADMIN_USER}"))
